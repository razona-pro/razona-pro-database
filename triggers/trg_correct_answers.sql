DROP TRIGGER IF EXISTS trg_update_tried_answers
ON razonapro.students_responses;

DROP TRIGGER IF EXISTS trg_update_ai_tried_answers
ON razonapro.ai_tried_responses;
...
CREATE FUNCTION razonapro.fn_update_tried_correct_answers()
RETURNS TRIGGER AS $trg_update_tried_answers$
DECLARE
...
END;
$trg_update_tried_answers$ LANGUAGE plpgsql;

-- Recuenta correct_answers en AI_TRIEDS cada vez que se inserta,
-- actualiza o elimina una respuesta en AI_TRIED_RESPONSES
CREATE FUNCTION razonapro.fn_update_ai_tried_correct_answers()
RETURNS TRIGGER AS $trg_update_ai_tried_answers$
DECLARE
    v_program_id VARCHAR(3);
    v_student_id VARCHAR(7);
...
BEGIN
    ...
    RETURN COALESCE(NEW, OLD);
END;
$trg_update_ai_tried_answers$ LANGUAGE plpgsql;

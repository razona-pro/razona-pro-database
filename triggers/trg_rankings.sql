DROP FUNCTION IF EXISTS razonapro.fn_refresh_student_ranking(VARCHAR, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS razonapro.fn_update_ranking_on_tried() CASCADE;
DROP FUNCTION IF EXISTS razonapro.fn_update_ranking_on_ai_tried() CASCADE;
    ...
    LOOP
        CASE v_ranking.period_type
            ...
            WHEN 'MONTHLY' THEN
                ...
                v_period_end   := (DATE_TRUNC('month', CURRENT_DATE)
                                    + INTERVAL '1 month'
                                    - INTERVAL '1 day')::DATE;
            ELSE
                v_period_start := NULL;
...
-- Dispara el recalculo de ranking cuando un TRIED pasa a FINISHED
...
-- Dispara el recalculo de ranking cuando un AI_TRIED pasa a FINISHED
